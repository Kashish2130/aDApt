...
const LostNFoundPage = () => {
  const [categories, setCategories] = useState([
    { id: "lab", name: "Lab", items: [] },
    { id: "cep", name: "CEP", items: [] },
    { id: "resource_center", name: "Resource Center", items: [] },
    { id: "canteen", name: "Canteen", items: [] },
    { id: "sac", name: "SAC", items: [] },
    { id: "basketball_court", name: "Basketball Court", items: [] },
  ]);
  ...
  const addCategory = () => {
    if (newCategory.trim()) {
      setCategories([...categories, { id: newCategory.toLowerCase().replace(/ /g, "_"), name: newCategory, items: [] }]);
      setNewCategory("");
    }
  };
    ...
    <div className="flex h-screen bg-[#F5F0CD]">
      <div className="w-1/4 bg-[#81BFDA] p-6 shadow-xl">
        ...
        <div className="mb-4 flex">
          ...
          />
          <button onClick={addCategory} className="ml-2 bg-green-500 text-white px-4 py-2 rounded-lg">Add</button>
        </div>
                ...
                <>
                  <button
                    className={`w-full text-left text-lg font-semibold px-4 py-3 rounded-lg transition ${
                      selectedCategory === category.id ? "bg-[#FADA7A] text-gray-900" : "bg-[#B1F0F7] text-gray-800 hover:bg-[#FADA7A]"
                    }`}
      ...
      <div className="flex-1 p-10">
        ...
        {selectedCategory ? (
          <motion.div className="mt-6 p-6 bg-white shadow-lg rounded-xl">
            ...
            <div className="mb-4 flex">
              <input
                ...
                className="p-2 border rounded-lg w-full"
              />
              <button onClick={addItem} className="ml-2 bg-blue-500 text-white px-4 py-2 rounded-lg">Add</button>
            </div>
          ...
          <p className="text-lg text-gray-700 mt-6">Select a location to view items.</p>
        )}
      </div>
    </div>
  );
